namespace SingleServiceApp.Providers.Auth.Security
{
    public class DesEncryption : IEncryptionAlgorithm
    {
        ...
        {
            Key = new byte[8] { 0, 0, 0, 0, 0, 0, 0, 0 };
            for (int i = 0; i < 8; i++)
                if (i < key.Length)
                    Key[i] = (byte)key[i];
                else
        ...
        {
            return Crypt(input, 0);
        }

        public string Decryption(string input)
        {
            var output = Crypt(input, 1);
            var res = output.Replace("\0", "");
            return output.Replace("\0", "");
        }

        private string Crypt(string input, uint mode)
        {
            byte[,] schedule = new byte[16, 6];

            string output = "";

            int del = input.Length % 8 == 0 ? 0 : 1;
            int countBlock = input.Length / 8 + del;
            byte[][] inputByte = new byte[countBlock][];
            byte[][] outputByte = new byte[countBlock][];

            for (int i = 0; i < countBlock; i++)
            {
                inputByte[i] = new byte[8];
                outputByte[i] = new byte[8];
            }

            for (int i = 0; i < countBlock; i++)
                for (int j = 0; j < 8; j++)
                    if (i * 8 + j < input.Length)
                        inputByte[i][j] = (byte)input[i * 8 + j];
                    else
                        inputByte[i][j] = 0;

            DesAlgorithm.KeySchedule(Key, schedule, mode);

            var juggedArray = ToJaggedArray(schedule);

            for (int i = 0; i < countBlock; i++)
            {
                DesAlgorithm.Crypt(inputByte[i], outputByte[i], juggedArray);
                for (int j = 0; j < 8; j++)
                    output += (char)outputByte[i][j];
            }
    ...
    internal static class DesAlgorithm
    {
        const int DECRYPT = 0;
